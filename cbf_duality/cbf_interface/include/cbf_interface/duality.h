...
protected:
  ...
  {
    ad_matrix_t ret(info_.num_points, 2);
    for (int p = 0; p < info_.num_points; ++p)
      ret.row(p) = parameters.segment<2>(o * 3 * info_.num_points + p * 2);
    return ret;
  }

  ad_vector_t getObstacleB(const ad_vector_t& parameters, size_t o) const
  {
    return parameters.segment(o * 3 * info_.num_points + info_.num_points * 2, info_.num_points);
  }
...
public:
  DualityLagrangianAd(const DualityObstacles& obstacles, const DualityInfo& info) : DualityBaseAd(obstacles, info)
  {
    initialize(info.centroidal_info.stateDim, info.centroidal_info.inputDim, obstacles.getParametersSize(),
               "DualityLagrangian", "/tmp/ocs2", true, false);
  }
                                 ...
                                 const ad_vector_t& parameters) const override
  {
    ...
    //    constraint(0) = (-ad_scalar_t(0.25) * lambda.tail(num_points_).transpose() * getObstacleA(parameters) *
    //                         getObstacleA(parameters).transpose() * lambda.tail(num_points_) -
    //                     lambda.head(4).transpose() * robot.getB() -
    //                     lambda.tail(num_points_).transpose() * getObstacleB(parameters))(0) -
    //                    0.05;
...
public:
  ...
  {
    initialize(info.centroidal_info.stateDim, info.centroidal_info.inputDim, obstacles.getParametersSize(),
               "DualityVector", "/tmp/ocs2", true, false);
  }
  size_t getNumConstraints(ocs2::scalar_t time) const override
  {
    return 2 * info_.num_obstacles;
  }
  ad_vector_t constraintFunction(ad_scalar_t time, const ad_vector_t& state, const ad_vector_t& input,
                                 const ad_vector_t& parameters) const override
  {
    ...
    ad_vector_t constraint(2 * info_.num_obstacles);
  ...
  DualityVectorAd* clone() const override
  {
    return new DualityVectorAd(*this);
  }
...
public:
  ...
  {
    initialize(info.centroidal_info.stateDim, info.centroidal_info.inputDim, obstacles.getParametersSize(),
               "DualityPositive", "/tmp/ocs2", true, false);
  }
  size_t getNumConstraints(ocs2::scalar_t time) const override
  {
    return info_.num_obstacles * 4 + info_.num_obstacles * info_.num_points;
  }
  ad_vector_t constraintFunction(ad_scalar_t time, const ad_vector_t& state, const ad_vector_t& input,
                                 const ad_vector_t& parameters) const override
  {
  ...
  DualityPositiveAd* clone() const override
  {
    return new DualityPositiveAd(*this);
  }
...
public:
  ...
  {
    initialize(info.centroidal_info.stateDim, info.centroidal_info.inputDim, obstacles.getParametersSize(),
               "DualityNorm", "/tmp/ocs2", true, false);
  }
  size_t getNumConstraints(ocs2::scalar_t time) const override
  {
    return info_.num_obstacles;
  ...
  DualityNormAd* clone() const override
  {
    return new DualityNormAd(*this);
  }
